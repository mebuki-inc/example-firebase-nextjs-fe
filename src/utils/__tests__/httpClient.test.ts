...
describe('httpClient', () => {
  ...
  describe('mutate', () => {
    ...
    test.each`
      ...
      ${'http://localhost'} | ${'post'}   | ${'/api/v1/users'}   | ${requestHeader} | ${body} | ${200} | ${stubResponseHeaders} | ${stubMutateResponseBody} | ${success}             | ${'POSTに成功した場合、{ status, body, headers, error: false } が正常に返却される'}
      ${'http://localhost'} | ${'put'}    | ${'/api/v1/users/1'} | ${requestHeader} | ${body} | ${200} | ${stubResponseHeaders} | ${stubMutateResponseBody} | ${success}             | ${'PUTに成功した場合、{ status, body, headers, error: false } が正常に返却される'}
      ${'http://localhost'} | ${'patch'}  | ${'/api/v1/users/1'} | ${requestHeader} | ${body} | ${200} | ${stubResponseHeaders} | ${stubMutateResponseBody} | ${success}             | ${'PATCHに成功した場合、{ status, body, headers, error: false } が正常に返却される'}
      ${'http://localhost'} | ${'delete'} | ${'/api/v1/users/1'} | ${requestHeader} | ${body} | ${200} | ${stubResponseHeaders} | ${stubMutateResponseBody} | ${success}             | ${'DELETEに成功した場合、{ status, body, headers, error: false } が正常に返却される'}
